...
export async function GET(
  ...
  try {
    ...
    if (!resenaExistente) {
      return jsonOk({ resena: null });
    }

    return jsonOk({
      resena: {
        puntaje: resenaExistente.puntaje,
...
export async function POST(
  ...
  try {
    ...
    if (puntaje === undefined) {
      return NextResponse.json(
        { error: "No se especifica puntaje" },
        { status: 400 },
      );
    }
    ...
    if (!Number.isInteger(puntaje)) {
      return NextResponse.json(
        { error: "Puntaje debe ser un numero entero" },
        { status: 400 },
      );
    }
    ...
    if (puntaje < 1 || puntaje > 5) {
      NextResponse.json(
        { error: "Puntaje debe ser entre 1 y 5" },
        { status: 400 },
      );
    }
    ...
    if (envio.estado !== "ENTREGADO") {
      return NextResponse.json(
        { error: "El envio no esta en estado ENTREGADO" },
        { status: 400 },
      );
    }

    const resenaExistente = await prisma.resena.findFirst({
      where: { envioId: Number(id) },
    });

    if (resenaExistente) {
      return NextResponse.json(
        { error: "El envio ya posee una resena registrada" },
        { status: 409 },
      );
    }

    const resena = await prisma.resena.create({
      data: { envioId: Number(id), puntaje: puntaje, comentario: comentario },
    });
